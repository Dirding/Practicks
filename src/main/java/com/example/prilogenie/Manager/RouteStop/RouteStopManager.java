...
public class RouteStopManager extends BaseManager<Route> { // Контроллер управления остановками маршрутов
    ...
    protected void setupCols() { // Настройка колонок таблицы
        ...
        table.setRowFactory(tv -> new TableRow<Route>() {
            @Override protected void updateItem(Route i, boolean e) {super.updateItem(i, e);}
        });
    }
    ...
    @Override
    protected void setupActCol() { // Настройка колонки с кнопкой "Остановки"
        if (actCol != null) {
            actCol.setCellFactory(col -> new TableCell<Route, Void>() {
                private final Button btn = new Button("Остановки");
                {
    ...
    @Override
    protected void showDialog(Route r) { // Открытие диалога управления остановками
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/prilogenie/Manager/RouteStop/dialog_routestop.fxml"));
            VBox page = loader.load();
            Dialogroutestop ctrl = loader.getController();
            ctrl.setRoute(r);
            ctrl.setRouteStopManager(this);
            Stage dlg = new Stage();
    ...
    @Override protected void saveToDB(Route e, boolean u) throws SQLException {} // Сохранение. Реализация пустая, потому что интерфейс требует объявить
    ...
    @FXML public void handleRefresh() { loadStops(); refreshData(); Alert.success("Список обновлён"); } // Обновление списка маршрутов
    public void refreshRoutes() { loadStops(); refreshData(); } // Обновление данных (вызывается из диалога после сохранения остановок)
}
